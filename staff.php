...
<?php endif; ?>
      ...
      <table class="table" id="staffTable">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><span class="badge <?php echo $roles[$user['role']]['badge']; ?> rounded" style="padding:6px 14px;"><i class="fa-solid <?php echo $roles[$user['role']]['icon']; ?>"></i> <?php echo $user['role']; ?></span></td>
            <td><?php echo date('d M Y', strtotime($user['created_at'])); ?></td>
            <td>
              <button class="btn btn-warning btn-sm editStaffBtn" data-id="<?php echo $user['user_id']; ?>"><i class="fa-solid fa-pen"></i></button>
              <?php if ($user['user_id'] != $_SESSION['user_id']): ?>
              <button class="btn btn-danger btn-sm deleteStaffBtn" data-id="<?php echo $user['user_id']; ?>"><i class="fa-solid fa-trash"></i></button>
              <?php endif; ?>
            </td>
          </tr>
          <?php endforeach; ?>
  ...
  <div id="staffModal" class="modal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.18); z-index:999; align-items:center; justify-content:center;">
    <div class="card shadow" style="max-width:400px; width:100%; position:relative;">
      <button id="closeStaffModal" style="position:absolute; top:12px; right:12px; background:none; border:none; font-size:1.3rem; color:var(--danger); cursor:pointer;"><i class="fa-solid fa-xmark"></i></button>
      ...
      <form id="staffForm" method="post" action="staff.php">
        ...
        <label for="modal_username">Username <i class="fa-solid fa-circle-question text-info" title="Enter the staff username."></i></label>
        <input type="text" name="username" id="modal_username" required>
        <label for="modal_password">Password <i class="fa-solid fa-circle-question text-info" title="Set a password. Leave blank to keep current (edit only)."></i></label>
        <input type="password" name="password" id="modal_password">
        <label for="modal_role">Role <i class="fa-solid fa-circle-question text-info" title="Select the staff role."></i></label>
        ...
        <div class="flex flex-between mt-2">
          <button type="submit" class="btn btn-accent" id="saveStaffBtn" name="add_staff"><i class="fa-solid fa-save"></i> Save</button>
          <button type="button" class="btn btn-danger" id="cancelStaffBtn"><i class="fa-solid fa-xmark"></i> Cancel</button>
        </div>
      </form>
    </div>
  </div>
